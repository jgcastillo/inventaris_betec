...
async def create_user(
    ...
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:create-user")),
) -> ServiceResult:
...
async def get_user_by_id(
    ...
    db: Database = Depends(get_database),
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:get-user-by-id")),
) -> ServiceResult:
...
@router.get(
    "/", response_model=Dict, name="users:users_list", status_code=status.HTTP_200_OK
)
async def get_users_list(
    ...
    direction: str = "",
    db: Database = Depends(get_database),
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:users_list")),
) -> ServiceResult:
...
async def update_user_by_id(
    ...
    db: Database = Depends(get_database),
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:update-user-by-id")),
) -> ServiceResult:
...
@router.put(
    "/activate/{id}", response_model=UserPublic, name="users:activate-user-by-id"
)
async def activate_user_by_id(
    id: UUID = Path(..., title="The id of the user to update"),
    user_update: UserActivate = Body(..., embed=True),
    db: Database = Depends(get_database),
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:activate-user-by-id")),
) -> ServiceResult:
...
async def delete_user_by_id(
    ...
    db: Database = Depends(get_database),
    current_user: UserInDB = Depends(GetCurrentActiveUser("users:delete-user-by-id")),
) -> ServiceResult:
...
@router.put(
    ...
    name="users:change-password-by-id",
)
async def change_password_by_id(
    ...
    current_user: UserInDB = Depends(
        GetCurrentActiveUser("users:change-password-by-id")
    ),
) -> ServiceResult:
    """
