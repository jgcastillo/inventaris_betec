...
class UserService(AppService):
    async def create_user(self, user: UserToSave) -> ServiceResult:
        ...
        if await user_repo.get_user_by_username(user.username):
            logger.error(
                f"Try to create a User with an existing username: {user.username}"
            )
        ...
        user_item: UserInDB = await user_repo.create_user(user)
        if not user_item:
    ...
    async def get_user_by_id(self, id: UUID) -> ServiceResult:
        user: UserInDB | dict = await UserRepository(self.db).get_user_by_id(id=id)
    ...
    async def update_user(
        ...
        try:
            ...
            user_public = await self.__get_updated_user(
                id=id,
            ...
            return ServiceResult(UserExceptions.UserInvalidUpdateParamsException(e=e))
    ...
    async def activate_user(
        ...
        try:
            ...
            return ServiceResult(user_public)
        except UserExceptions.UserNotFoundException:
            return ServiceResult(UserExceptions.UserNotFoundException())
    ...
    async def __get_updated_user(
        self,
        id: UUID,
        user_update: UserUpdate | UserActivate,
        updated_by_id: UUID,
        credentials: dict,
    ) -> UserPublic:
        user: UserInDB = await UserRepository(self.db).update_user(
            id=id,
            user_update=user_update,
