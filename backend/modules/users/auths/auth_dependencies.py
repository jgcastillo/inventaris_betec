...
from shared.core.config import API_PREFIX, SECRET_KEY
...
class GetCurrentActiveUser:
    def __init__(self, permission: str):
        self.permission = permission

    async def __call__(self, current_user: UserInDB = Depends(get_user_from_token)):
        if not current_user:
            raise AuthExceptions.AuthUnauthorizedException()
        if not current_user.is_active:
            raise AuthExceptions.AuthUnauthorizedException()

        if current_user.is_superadmin or self.permission in current_user.permissions:
            return current_user
        else:
            raise AuthExceptions.AuthUnauthorizedException()
