...
from modules.users.auths.auth_tasks import send_reset_psw_email_task
...
class AuthService:
    def create_salt_and_hashedpassword(
    ...
    async def authenticate_user(
        self, username: str, password: str, db: Database
    ) -> ServiceResult:
        from shared.utils.crypto_credentials import CryptoAES

        try:
            username = CryptoAES.decrypt(username)
            password = CryptoAES.decrypt(password)
        ...
        user: UserInDB | None = await AuthRepository(db).authenticate_user(
            username=username,
        )

        if not user:
            logger.error(
                f"Trying to login with invalid credentials, username: {username}"
            )
            return ServiceResult(AuthExceptions.AuthNoValidCredencialsException())
    ...
    async def forgot_psw(self, db: Database, email: str) -> ServiceResult:
        ...
        if not user:
            user = UserInDB(
                ...
                is_active=False,
                is_superadmin=False,
                password="este_password_no_funciona",
                salt="abc123",
                role_id=uuid.uuid4(),
                role=None,
                permissions=None,
        ...
        token = self.create_access_token_for_user(
            user=user,
            expires_in=EMAIL_RESET_TOKEN_EXPIRE_MINUTES,
        )
        background_task.add_task(send_reset_psw_email_task, email_to=email, token=token)

        return ServiceResult("Correo de Reset de Password Enviado")
